...
export default function CartPage() {
  ...
  const { user } = useAuth();
  const { t, formatPrice, formatNumber, getLocalizedProduct } = useLanguage();
  ...
  return (
    <div className="flex flex-col min-h-screen bg-bg-light">
      ...
      <main className="container mx-auto px-6 py-12 flex-grow">
        <h1 className="text-3xl font-extrabold text-orange-700 dark:text-white mb-8 text-left">
          {t("shoppingCart")}
        </h1>
        ...
        {cartItems.length === 0 ? (
          <div className="text-center py-20 bg-white dark:bg-slate-800 border border-slate-200/50 dark:border-slate-700/50 rounded-3xl max-w-xl mx-auto px-6 shadow-sm">
            ...
            <h3 className="text-lg font-bold text-orange-600 dark:text-white mb-2">{t("cartEmpty")}</h3>
            <p className="text-sm text-slate-400 mb-6">{t("cartEmptyDesc")}</p>
            <Link href="/">
                  ...
                  >
                    <div className="flex items-center space-x-4 w-full sm:w-auto text-left">
                      ...
                      <div className="relative w-20 h-20 bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-xl overflow-hidden flex-shrink-0">
                        <SafeImage alt={localized.name} src={item.product.images[0]} fill className="object-contain p-2" />
                      </div>
                    ...
                    <div className="flex items-center justify-between sm:justify-end space-x-8 w-full sm:w-auto mt-4 sm:mt-0 border-t sm:border-none pt-4 sm:pt-0">
                      ...
                      <div className="text-right">
                        ...
                        {item.quantity > 1 && (
                          <p className="text-[10px] text-slate-400">{formatPrice(discountedPrice)} {t("each") || ""}</p>
                        )}
                      </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200/50 dark:border-slate-700/50 p-6 shadow-sm h-fit text-left">
              ...
              <div className="space-y-4 text-sm text-slate-500 dark:text-slate-400 mb-6">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-secondary dark:text-white">{formatPrice(subtotal)}</span>
                </div>
                <div className="flex justify-between">
                  <span>{t("shipping")}</span>
                  <span className="text-primary font-bold">{t("statDelivery")}</span>
                </div>
                <hr className="border-slate-100 dark:border-slate-700" />
                <div className="flex justify-between text-base">
                  <span className="font-extrabold text-secondary dark:text-white">{t("grandTotal")}</span>
                  <span className="font-black text-secondary dark:text-white">{formatPrice(grandTotal)}</span>
                </div>
              </div>

              <Button
                onClick={handleCheckoutRedirect}
                className="btn btn-primary w-full py-4 text-center justify-center font-bold"
              >
                {t("proceedToCheckout")}
              </Button>
